...
3. Toggles:
    ...
    - Enable strict mode (only if a start event is provided): If a start node is provided, enabling strict mode will remove all components of the graph that are not connected to the start node through extra post-processing. This toggle is turned off by default.
