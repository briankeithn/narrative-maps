...
def create_RLP(query, sim_table, membership_vectors, clust_sim_table, exp_temp_table, ent_table, numclust, relevance_table,
    ...
    for idx, op in enumerate(operations):
        ...
        if "AN" in op:
            ...
            if not replace[idx]:
                ...
                prob += node_act_vars[str(op_ref)] >= min_threshold_node, 'AddedNode' + str(op_ref) # Makes problem infeasible if we add too many.
        elif "AE" in op:
            if verbose:
    ...
    if force_cluster:
        min_threshold_edge = 0.05 # Default values.
        min_threshold_node = 0.01 # Default values.
...
def compute_sim_with_t(query, dataset, sigma_t=30, min_dist=0.0): # Assume no temporal similarity by default.
...
def get_entity_table(query, dataset):
    ...
    for index, article in query.iterrows():
        ...
        doc_list.append(doc)
        ent_doc_list.append([ent for ent in doc.ents])
...
def solve_LP(query, dataset,
    ...
    umap_init='spectral', cosine_sim = True):
