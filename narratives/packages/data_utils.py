...
def clean_publication(graph_df: pd.DataFrame) -> pd.DataFrame:
...
def is_path_edge(i: int | str, j: int | str, val_list: list[str]) -> bool:
    if len(val_list) == 0:
        return False
...
def centroid_tuples(arr: npt.NDArray[np.float_]) -> (float, float):
    length = arr.shape[0]
    sum_x = np.sum(arr[:, 0])
    sum_y = np.sum(arr[:, 1])
...
def generate_elements(graph_df: pd.DataFrame, antichain: list[str], hub_nodes: list[str], sp: list[str], low_dim_projection: dict[int, npt.NDArray[Any]] | None = None,
                      query: pd.DataFrame | None = None, graph_precomputed_positions: dict[int, tuple[float, float]] | list[Any] = [], story_list: list[list[str]] = [], story_names: list[list[str]] = [], unraveled_set: set[int] = set(),
                      elements: list[Any] = [], label_length: int = 450) -> list[dict[str, bool | str] | bool | str]:
    ...
    for index, row in graph_df.iterrows():
        ...
        cur_dict = {"data": {"id": str(row['id']), "parent": "story_" + str(story_data_tuple[0]), "label": label, "full_text": row['title'], "date": date, "source": source,
                             "type": "in_map", "story": story_data_list, "storyname": story_name_str[0]}, "classes": "n"}
...
def add_execution_id(element_list: list[dict[str, dict[str, bool | int | str] | bool | str]], execution_id: int) -> list[dict[str, dict[str, bool | int | str] | bool | str]]:
...
def remove_execution_id(element_list: list[dict[str, dict[str, bool | int | str] | bool | str]], execution_id: int) -> list[dict[str, dict[str, bool | int | str] | bool | str]]:
    modified_exec_id = "_" + str(execution_id) + "I"
    for ele in element_list:
...
def read_query(dataset: str, start_date: datetime | None = None, end_date: datetime | None = None, partial: bool = False) -> pd.DataFrame:
    if partial:
...
def read_query_search(dataset, start_date: datetime | None = None, end_date: datetime | None = None):
    query = pd.read_csv("../data/" + str(dataset) + '_search.csv')
    query = clean_publication(query)
