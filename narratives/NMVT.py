...
toggle_table = dbc.Table([html.Tbody([toggle_row_ent, toggle_row_temp, toggle_row_si, toggle_row_xai, toggle_row_names, toggle_regularization, toggle_strict_start])],
                         bordered=False, borderless=True, style={'vertical-align': 'middle'})
        ...
        dbc.Col(width=4, children=[
            dcc.Tabs(id='tabs', value="0", style={'font-size': 12, 'height': '5.5vh'}, children=[
                dcc.Tab(label='Overview', value="0",
                        ...
                        selected_style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center',
                                        'word-spacing': '100vw'}, children=[
                        html.Div(id='overview-tab', style=styles['tab'], children=[
                            html.Div(
                                'This tab provides an overview of the data set through its topic and most relevant entities. It also contains a 2D visualization of the document space and the main storyline of the map as it navigates through this space.'),
                ...
                dcc.Tab(label='Event Details', value="1",
                        style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center', 'word-spacing': '100vw'},
                        selected_style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center',
                                        'word-spacing': '100vw'}, children=[
                ...
                dcc.Tab(label='Edge Details', value="2",
                        style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center', 'word-spacing': '100vw'},
                        selected_style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center',
                                        'word-spacing': '100vw'}, children=[
                        html.Div(style=styles['tab'], children=[
                ...
                dcc.Tab(label='Event Comparison', value="3",
                        style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center', 'word-spacing': '100vw'},
                        selected_style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center',
                                        'word-spacing': '100vw'}, children=[
                        html.Div(style=styles['tab'], children=[
                ...
                dcc.Tab(label='Data Set', value="4",
                        style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center', 'word-spacing': '100vw'},
                        selected_style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center',
                                        'word-spacing': '100vw'}, children=[
                        html.Div(id='data-table-tab', style=styles['tab'], children=[
                            html.Div(
                                'Data table with all the events from the current data set. You can search for specific events here to add them to the map (select rows and add them to the map). You may also set a single starting event (use the radio button of the row you want to set as the starting event).'),
                ...
                dcc.Tab(label='Assistant', value="6",
                        style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center', 'word-spacing': '100vw'},
                        selected_style={'padding': '0', 'font-size': '0.7vw', 'display': 'flex', 'align-items': 'center', 'justify-content': 'center',
                                        'word-spacing': '100vw'}, children=[
                        html.Div(style=styles['tab'], children=[
...
def interact_with_graph(rmv_node: int | None, rmv_edge: int | None, add_edge: int | None, add_node: int | None,
                        # main_route, anti_chain,
                        recompute_lp: int | None, add_cluster_list: int | None, search_btn: int | None, search_enter_key: int | None, query: pd.DataFrame,
                        elements: list[object] | None, nodes: list[object] | None, edges: list[object] | None, k_input: int | None, mincover_input: int | None,
                        sigma_t: int | None,  # trans_reduce,
                        dataset: str, cluster_value: int | None, search_value: str | None, similar_input: int,
                        selected_rows_table: list[int], selected_cells: list[Any] | None, scatter_fig: list[Any] | None, execution_id: int, xai_tab: list[Any],
                        overview_tab: list[Any], previous_actions: list[Any],
                        use_entities: bool, use_temporal: bool, use_si: bool, use_xai: bool, use_names: bool, use_regularization: bool, strict_start: bool) -> list[
    Any]:  # , rep_landmark_mode):
    ...
    if button_id == 'remove-node-button':
        if elements and nodes:
            ...
            if len(actual_removed_nodes) > 0:
                new_elements = [ele for ele in elements if ele['data']['id'] not in ids_to_remove and ele['data'].get('target') not in ids_to_remove and ele['data'].get(
                    'source') not in ids_to_remove]  # Created new list.
        ...
        graph_df_new, status, scatter_df, sim_table, clust_sim_table, ent_table, ent_doc_list, cluster_assignment = solve_LP(query,
                                                                                                                             ...
                                                                                                                             sigma_t=sigma_t,
                                                                                                                             min_samples=1, min_cluster_size=cluster_size_est,
        ...
        if 'Optimal' in status[1]:
            ...
            if use_xai:  # No topic or entity details if XAI is disabled.
                ...
                # Process topic lists
                topic_list = [html.P([html.B("Topic " + str(idx_topic) + ": "), html.Span(desc)], style={'fontSize': 14}) for idx_topic, desc in
                              enumerate(cluster_description)]
                # Process entities
                ...
                return [add_execution_id(new_elements, execution_id), base_layout_fit, html.P([status_msg]), previous_actions, scatter_fig, execution_id, new_xai_table,
                        new_overview_tab]
            else:
                new_overview_tab = [html.Div('Topic list', style={'fontSize': 16, 'fontWeight': 'bold', 'text-decoration': 'underline'}),
                ...
                return [add_execution_id(new_elements, execution_id), base_layout_fit, html.P([status_msg]), previous_actions, scatter_fig, execution_id, xai_tab,
                        new_overview_tab]
...
@app.callback([Output('tap-edge', 'children'),
              ...
              [State('execution-id', 'value'),
               ...
               State('store', 'data'),
               State('use-xai', 'on')])
def explainEdge(cmp_btn: int | None, data: None, execution_id: int, xai_tab: list[dict[str, list[float | str] | int | str]], edge_data: list[Any] | None, query: pd.DataFrame,
                use_xai: bool) -> list[list[Component] | str]:
    ctx = dash.callback_context
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    ...
    if button_id == 'explain-edge':
        if not use_xai:
            output_list += [
                html.P(children=[html.Strong('The XAI component is disabled, please enable and re-generate the map to get connection explanations.')], style={'fontSize': 16})]
            return [output_list, ""]
        if len(edge_data) == 1:  # Only one edge must be selected.
            ...
            if str(id_end) not in adjacency_list_start:
                ...
                explanation += [html.Strong("Keyword contributions to the connection", style={'fontSize': 16, 'text-decoration': 'underline'}),
            ...
            if "Manual" in data['label']:  # No more temporal connections.
                ...
                if "Similarity" in data['label']:
                    explanation += [html.P("This connection is based on similar content and keywords. See extracted keywords and their contributions to similarity below.",
                                           style={'fontSize': 14})]
...
@app.callback([Output('tap-node-id', 'children'),
              ...
              [State('execution-id', 'value'),
               ...
               State('cytoscape', 'selectedNodeData'),
               State('store', 'data')])
def displayTapNodeData(data: pd.DataFrame | None, execution_id: int, xai_tab: list[dict[str, list[float | str] | int | str]], node_data: list[Any] | None,
                       query: pd.DataFrame) -> list[list[Component] | str]:
    if data:
        if "story" in data['id']:
            return ["", "", "", "", ""]
        # query = read_query(dataset, start_date, end_date)
...
def run_chatbot(n_clicks: int, n_submit: int | None, explain_edge_chat: int, explain_important_chat: int, explain_story_chat: int, execution_id: int, query: pd.DataFrame,
                user_input: str, chat_history: str, elements: list[dict[str, dict[str, bool | int | str]]] | None, edge_data: list[Any] | None,
                node_data: list[Any] | None) -> (str, None):
    ...
    if changed_id == "submit" or changed_id == "user-input":
        ...
        if len(node_data) == 1:  # Only one node must be selected.
            ...
            user_input = "Explain why the event (" + node_data[0]['label'] + ") is important to the storyline. Consider its preceding events: (" + ", ".join(
                edge_i_event_labels) + ") and its subsequent events (" + ", ".join(edge_j_event_labels) + ")."
            chat_history += f"You: {user_input}<split>{name}: "
        else:
